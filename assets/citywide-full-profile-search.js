(function(){
  ...
  var LU_FAMILY={'01':'residential','02':'residential','03':'residential','04':'residential and commercial',
    '05':'commercial','06':'industrial','07':'transportation or utility','08':'a public facility or institution',
    '09':'open space or outdoor recreation','10':'parking','11':'vacant'};
  function zoneFamily(z){z=String(z||'').trim().toUpperCase();
    if(z.indexOf('/')>-1) return 'mixed use';
    if(/^R/.test(z)) return 'residential'; if(/^C/.test(z)) return 'commercial';
    if(/^M/.test(z)) return 'industrial'; if(z==='PARK') return 'parkland'; return '';}
  function zoneUseGap(z,code,label){
    var key=String(code||'').trim().padStart(2,'0');
    var lu=LU_FAMILY[key], zf=zoneFamily(z);
    if(!lu||!zf||lu===zf) return '';
    if(zf==='mixed use'&&lu!=='open space or outdoor recreation'&&lu!=='parking'&&lu!=='vacant') return '';
    if(zf==='residential'&&lu==='residential and commercial') return '';
    return 'The zoning and the current use are two different things, and on this lot they do not match. '+
      z+' is what the zoning map allows here. '+(label||'The recorded land use')+
      ' is what the city records as being here now. A lot can be zoned '+zf+' and be used for '+lu+
      ': the use can predate the zoning, sit on public land, be something the district permits anyway, or hold a variance. '+
      'The zoning is what would govern if the site were redeveloped.';
  }
