(function(){
  ...
  function useGroupHero(code){
    var v=UG_BY_LANDUSE[String(code||'').trim().padStart(2,'0')];
    if(!v) return '';
    var txt=/^[IVX]+( and [IVX]+| or [IVX]+)? \u00b7 /.test(v) ? 'use group '+v : v;
    return '<div style="font-family:\'DM Mono\',monospace;font-size:.66rem;'
         + 'color:rgba(255,255,255,.82);margin-top:5px">'+esc(txt)+'</div>';
  }
