...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .install-app-btn { display: inline-flex; align-items: center; justify-content: center; gap: 6px; background: var(--orange); border: none; color: #fff; font-family: 'DM Sans', sans-serif; font-weight: 800; font-size: 0.8rem; letter-spacing: 0.3px; padding: 7px 14px; border-radius: 20px; cursor: pointer; transition: transform 0.15s ease, background 0.15s ease; -webkit-appearance: none; appearance: none; }
  .install-app-btn:hover { transform: translateY(-1px); background: #ff8a35; }
  .install-app-btn:active { transform: scale(0.98); }
  .install-app-btn .install-icon { font-size: 0.95rem; line-height: 1; }
  .install-modal { display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.72); z-index: 9999; align-items: center; justify-content: center; padding: 20px; }
  .install-modal.open { display: flex; }
  .install-modal-card { background: var(--navy); border: 2px solid var(--orange); border-radius: 18px; padding: 22px 20px 20px; max-width: 380px; width: 100%; box-shadow: 0 16px 40px rgba(0,0,0,0.6); position: relative; }
  .install-modal-close { position: absolute; top: 10px; right: 12px; background: none; border: none; color: rgba(255,255,255,0.7); font-size: 1.5rem; cursor: pointer; line-height: 1; padding: 4px 8px; }
  .install-modal-title { color: #fff; font-size: 1.1rem; font-weight: 800; margin-bottom: 6px; }
  .install-modal-sub { color: rgba(255,255,255,0.6); font-size: 0.82rem; margin-bottom: 16px; }
  .install-section { margin-bottom: 14px; padding: 12px 14px; background: rgba(255,255,255,0.05); border-radius: 10px; }
  .install-section h4 { color: var(--orange); font-size: 0.82rem; font-weight: 800; text-transform: uppercase; letter-spacing: 0.6px; margin-bottom: 8px; }
  .install-section ol { color: #fff; font-size: 0.86rem; line-height: 1.5; padding-left: 18px; }
  .install-section ol li { margin-bottom: 4px; }
  .install-section strong { color: var(--orange); }
...
<div class="topbar">
  ...
  <div class="topbar-right">
    <button id="install-app-btn" class="install-app-btn" type="button">
...
<div id="install-modal" class="install-modal" role="dialog" aria-modal="true" aria-labelledby="install-modal-title">
  <div class="install-modal-card">
    <button id="install-modal-close" class="install-modal-close" type="button" aria-label="Close">&times;</button>
    <div id="install-modal-title" class="install-modal-title">Install bkcb6.app</div>
    <div class="install-modal-sub">Add this to your home screen to open it like a regular app.</div>
...
(function(){
  ...
  function openM(){ modal.classList.add('open'); document.body.style.overflow = 'hidden'; }
